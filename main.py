...
connection = pymysql.connect(host='localhost',user='root',password='Sinjo@123',db='cv')
...
def run():
    ...
    link = '[©Developed by Vaibhav Singh Thapli ](https://www.linkedin.com/in/vaibhavthapli/)'
    ...
    if choice == 'User':
        ...
        if st.button('Login'):
            if ad_user == 'Vaibhav' and ad_password == 'Sinjo@123':
                st.success("Welcome Dr Vaibhav !")
                # Display Data
                ...
                print(label)
                value= plot_data.Predicted_Field.value_counts()
                print(value)
                st.subheader("**Pie-Chart for Predicted Field Recommendation**")
                fig = px.pie(df, values=value, names=label, title='Predicted Field according to the Skills')
                st.plotly_chart(fig)
